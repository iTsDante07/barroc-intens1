    ...
    <body class="bg-gray-900 text-white">
        ...
        <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
            <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
                <!-- Hero Section -->
                <div class="flex flex-col gap-6 lg:gap-10 lg:flex-1 lg:justify-center">
                    <div class="flex flex-col gap-4">
                        <h1 class="text-4xl lg:text-6xl font-bold text-white leading-tight">
                            Barroc <span class="text-[#ffd700]">Intens</span>
                        </h1>
                        <p class="text-lg lg:text-xl text-gray-300 leading-relaxed max-w-lg">
                            De ultieme koffie ervaring. Verse bonen, perfect gebrand, voor de echte koffieliefhebber. Ontdek onze intensieve smaken.
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        @if (Route::has('register'))
                            <a
                                href="{{ route('register') }}"
                                class="inline-flex items-center justify-center px-8 py-3 bg-[#ffd700] text-white font-semibold rounded-lg hover:bg-[#e6c200] transition-colors duration-200"
                            >
                                Bestel Nu
                            </a>
                        @endif
                        @if (Route::has('login'))
                            <a
                                href="{{ route('login') }}"
                                class="inline-flex items-center justify-center px-8 py-3 border-2 border-[#ffd700] text-[#919191] font-semibold rounded-lg hover:bg-[#ffd700] hover:text-white transition-all duration-200"
                            >
                                Inloggen
                            </a>
                        @endif
                ...
                <div class="flex items-center justify-center lg:flex-1 lg:justify-end">
                    <div class="relative">
                        <div class="w-64 h-64 lg:w-80 lg:h-80 bg-[#ffd700] rounded-full opacity-10 absolute -top-8 -right-8"></div>
                        <div class="w-48 h-48 lg:w-64 lg:h-64 bg-[#212121] rounded-lg shadow-2xl flex items-center justify-center relative z-10">
                            <div class="text-center">
                                <div class="w-16 h-16 lg:w-20 lg:h-20 bg-[#ffd700] rounded-full mx-auto mb-4 flex items-center justify-center">
                                    <svg class="w-8 h-8 lg:w-10 lg:h-10 text-[#212121]" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z"/>
                                    </svg>
                                </div>
                                <h3 class="text-[#ffd700] font-semibold text-lg lg:text-xl">Verse Koffie</h3>
                                <p class="text-gray-300 text-sm">Dagelijks gebrand</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @if (Route::has('login'))
            ...
            <html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
                <head>
                    ...
                    <!-- Vite compiled assets (styles + scripts) -->
                    @vite(['resources/css/app.css', 'resources/js/app.js'])
                    <style>
                        /* Minimal fallback to ensure readability if CSS fails to load */
                        body { background: #212121; color: #ffffff; font-family: 'Instrument Sans', system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; }
                        a { color: #ffffff; }
                    </style>
                </head>
                    ...
                    <div class="container mx-auto px-6 py-12">
                        <header class="flex items-center justify-between">
                            <a href="{{ url('/') }}" class="flex items-center gap-3">
                                <img src="{{ asset('images/Logo6_groot.png') }}" alt="Barroc Intens" class="h-12 w-auto" />
                                <span class="font-semibold text-lg text-white">Barroc <span class="text-[#ffd700]">Intens</span></span>
                            </a>

                            <nav class="flex items-center gap-4">
                                @if (Route::has('login'))
                                    <a href="{{ route('login') }}" class="px-4 py-2 rounded-md text-sm border border-transparent hover:border-[#ffd700]">Inloggen</a>
                                @endif
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="px-4 py-2 rounded-md bg-[#ffd700] text-white font-semibold">Bestel</a>
                                @endif
                            </nav>
                        </header>
                        ...
                        <main class="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                            <section class="lg:pr-12">
                                <h1 class="text-4xl lg:text-6xl font-bold leading-tight">
                                    Intensiteit in elke kop
                                    <span class="block text-[#ffd700]">Barroc Intens</span>
                                </h1>

                                <p class="mt-6 text-lg text-gray-300 max-w-xl">
                                    Ambachtelijk gebrande koffiebonen, met passie geselecteerd en zorgvuldig geroosterd. Ontdek onze single-origin selectie en signature blends.
                                </p>

                                <div class="mt-8 flex flex-col sm:flex-row gap-4">
                                    @if (Route::has('register'))
                                        <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-6 py-3 bg-[#ffd700] text-white rounded-md font-semibold shadow">Bestel Nu</a>
                                    @endif

                                    @if (Route::has('login'))
                                        <a href="{{ route('login') }}" class="inline-flex items-center justify-center px-6 py-3 border-2 border-[#ffd700] text-[#919191] rounded-md font-semibold hover:bg-[#ffd700] hover:text-white transition">Account</a>
                                    @endif
                                </div>

                                <div class="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div class="p-4 bg-[#1f1f1f] rounded-lg">
                                        <h4 class="font-semibold text-[#ffd700]">Verse bonen</h4>
                                        <p class="text-sm text-gray-300 mt-2">Dagelijks vers geroosterd voor maximale smaak.</p>
                                    </div>
                                    <div class="p-4 bg-[#1f1f1f] rounded-lg">
                                        <h4 class="font-semibold text-[#ffd700]">Snel bezorgen</h4>
                                        <p class="text-sm text-gray-300 mt-2">Snelle levering, thuis of op kantoor.</p>
                                    </div>
                                    <div class="p-4 bg-[#1f1f1f] rounded-lg">
                                        <h4 class="font-semibold text-[#ffd700]">Duurzaam</h4>
                                        <p class="text-sm text-gray-300 mt-2">Verantwoord geteeld en eerlijk gehandeld.</p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <aside class="flex items-center justify-center">
                                <div class="relative">
                                    <div class="w-72 h-72 lg:w-80 lg:h-80 bg-[#ffd700] rounded-full opacity-10 absolute -top-6 -right-6"></div>
                                    <div class="w-64 h-64 lg:w-72 lg:h-72 bg-[#212121] rounded-xl shadow-2xl flex items-center justify-center">
                                        <img src="{{ asset('images/Logo6_groot.png') }}" alt="Barroc Intens logo" class="w-36 h-36 object-contain" />
                                    </div>
                                </div>
                            </aside>
                        </main>

                        <footer class="mt-16 border-t border-gray-800 pt-6 text-sm text-gray-400">
                            <div class="flex items-center justify-between">
                                <p>&copy; {{ date('Y') }} Barroc Intens — Alle rechten voorbehouden</p>
